...
bool ImGui::MinimizeButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col);
    RenderNavCursor(bb, id, ImGuiNavRenderCursorFlags_Compact);
    ImU32 minus_col = GetColorU32(ImGuiCol_Text);
    ImVec2 center = bb.GetCenter();
    float line_length = g.FontSize * 0.6f;
    float line_thickness = 3.0f;

    ImVec2 p1 = center - ImVec2(line_length * 0.6f, 0.0f);
    ImVec2 p2 = center + ImVec2(line_length * 0.6f, 0.0f);

    window->DrawList->AddLine(p1, p2, minus_col, line_thickness);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    bool ret = Begin(name, NULL, NULL, window_flags);
    PopStyleVar();
    if (!ret)
    {
        EndPopup();
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    bool is_open = Begin(name, NULL, NULL, window_flags);
    PopStyleVar(2);
