...
void Helper::ParseFileList()
{
    std::string jsonContent = GetFileFromURL(0);
    ListaArquivos.clear();
...
std::string Helper::GetFileFromURL(int iType)
{
    ...
    std::string path = paths.count(iType) ? paths.at(iType) : "/filelist.json";
...
void Helper::FileCheckUpdate()
{
    ...
    std::for_each(std::execution::par_unseq, ListaArquivos.begin(), ListaArquivos.end(), [&](Arquivo& file) {
        sem.acquire(); // Aguarda permissão para processar
        ...
        sem.release(); // Libera permissão para outro thread
    });
...
bool Helper::DownloadFile(const std::string& remoteFile, const std::string& localPath, int fileIndex, int totalFiles)
{
    auto doDownload = [&](auto& cli) -> bool 
    {
        ...
        auto res = cli.Get(("/Update/" + remoteFile).c_str(),
...
void Helper::UpdateLauncher()
{
    ...
    std::string remoteLauncherHash = GetFileFromURL(2);
